...
public class PatternMatchingStudentTests {
    ...
    public void testBruteForceThere() {
    ...
    @Test(timeout = TIMEOUT)
    public void testBruteForceNotThere() {
        assertEquals(emptyList,
                PatternMatching.bruteForce(sell, sellNotThere, comparator));
        assertTrue("Did not use the comparator.", comparator.getCount() != 0);
        assertTrue("Comparison count was " + comparator.getCount()
                + ". Should be 36.", comparator.getCount() == 36);
    }

    @Test(timeout = TIMEOUT)
    public void testBruteForceLongerText() {
        assertEquals(emptyList,
                PatternMatching.bruteForce(sellNotThere, sell, comparator));
